...
bool BinaryFormat::checkHeader( QDataStream &stream ) const
{
  ...
  if ( !file ) {
    qCritical() << i18n( "Not a file?" );
    return false;
  }

  if ( magic != 0x2e93e ) {
    qCritical() << i18n( "File '%1' is not binary format.", file->fileName() );
    return false;
  }

  if ( version != BINARY_FORMAT_VERSION ) {
    qCritical() << i18n( "File '%1' is the wrong version.", file->fileName() );
    return false;
  }
...
/*
  ...
  if ( photo.isIntern() ) {
    ...
    if ( !img.load( locateLocal( "data", "kabc/photos/" ) + addressee.uid() ) )
      qDebug() << "No photo available for '" << addressee.uid() << "'.";
  ...
  if ( logo.isIntern() ) {
    ...
    if ( !img.load( locateLocal( "data", "kabc/logos/" ) + addressee.uid() ) )
      qDebug() << "No logo available for '" << addressee.uid() << "'.";
...
/*
  ...
  if ( photo.isIntern() ) {
    ...
    if ( !img.save( fileName, "PNG" ) )
      qDebug() << "Unable to save photo for '" << addressee.uid() << "'.";
  }
  ...
  if ( logo.isIntern() ) {
    ...
    if ( !img.save( fileName, "PNG" ) )
      qDebug() << "Unable to save logo for '" << addressee.uid() << "'.";
  }
